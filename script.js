...
new Slider("c", {
  ...
  autoplayTimer: 2, // If autoplay is true, trigger timer -- TIME as NUMBER in SECONDS (trans duration is added)
  /* Transition Options */
  duration: 2, // Transition Duration -- TRANSITION DURATION as NUMBER in SECONDS
  easeType: 6, // Interpolation Function, 0 is LINEAR, 12 is INOUTQUINT */
  type: 0,  // Slider Type: 0 = texture displacement (can change in html), 1 = pixel distance, 2 = TBD...
});
...
*** Is based on Martin Laxenaire Curtains.js (here: https://github.com/martinlaxenaire/curtainsjs).
*** The guy is amazing, the library is killer.
*** Star him on github. Hard. He's the genious, not me.


It's a basic implementation of a webgl slider with transition.


If you come from webflow, clone here: ...
...
* Note that using decimals might fuck up complex easings.
In general might require a bit of fiddling even when not using decimals.
Looking into solving this for next gen.
...
* Gsap might appear in future versions.

Implementation inspired by https://gist.github.com/gre/1650294.
...
* Geometry based onw will be implemented, thinking of possible other ones. Feel free to suggest.

!) Type 0 : transition based on a displacement map.
...
* If you understand code a bit you can change the shader by uncommenting two lines, as right to left is already (kinda) there.

!) Type 1 : pixel distance function.
